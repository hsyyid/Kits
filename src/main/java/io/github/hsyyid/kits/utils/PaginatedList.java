...
public class PaginatedList
{
	...
	public Text getPage(int pageNumber)
	{
		...
		for (Text item : items)
		{
			...
			{
				list.append(Texts.of(this.lineNumberColor, (itemIndex < 10) ? "0" + itemIndex + this.lineNumberType : Integer.toString(itemIndex) + this.lineNumberType));
			}
	...
	private Text getLink(String preview, int page)
	{
		...
		return Texts.builder(preview).onClick(TextActions.runCommand(this.command + " " + page)).onHover(TextActions.showText(Texts.of(TextColors.WHITE, "Go to page ", TextColors.GOLD, page))).color(this.clickableLinkColor).build();
